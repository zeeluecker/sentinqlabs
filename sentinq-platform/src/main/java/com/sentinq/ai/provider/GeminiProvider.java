...
public class GeminiProvider
    ...
    private GoalFitReasoningDecision deserializeGoalFitDecision(
        ...
        try {
            ...
            throw new IllegalStateException(
                    "Gemini returned invalid goal-fit reasoning JSON: "
                            + responseText,
                    exception
            );
        }
    ...
    private String buildGoalFitPrompt(
        ...
        5. Hard screening has already removed candidates that can be
           cheaply proven to be non-starters. Do not repeat that
           filtering step.

        6. Rank candidates based on apparent product fit to the
           consumer's original objective, context, and stated preferences.
        ...
        8. Do not infer or investigate current shipping cost, tax,
           inventory availability, or whether a delivery date can
           actually be met. If those facts are not explicitly present
           in the supplied candidate information, leave them unknown.

        9. Do not decide whether a purchase may be executed on the
           consumer's behalf. Do not reason about permissions, consent,
           spending authority, or whether the agent is allowed to
        ...
        13. offerId must exactly match the offerId of the supplied
            candidate being ranked.

        14. reasoning should briefly explain why the candidate occupies
            that position relative to the consumer's stated objective.

        15. Do not invent product attributes that are not present
            in the supplied candidate information.
        
        16. The offer object must reproduce the supplied CandidateOffer
              exactly. Do not change, infer, add, remove, or normalize any
              offer field or field value.
    ...
    private String formatGoalFitCandidates(
        ...
        for (int i = 0; i < candidates.size(); i++) {
            ...
            builder.append("- Merchant ID: ")
                    .append(candidate.getMerchantId())
                    .append("\n");
            ...
            builder.append("- Product price cents: ")
                    .append(candidate.getProductPriceCents())
                    .append("\n");

            builder.append("- Fulfillment score: ")
                    .append(candidate.getFulfillmentScore())
                    .append("\n");

            builder.append("- Review score: ")
                    .append(candidate.getReviewScore())
                    .append("\n\n");
        }
    ...
    private RecommendationReasoningDecision deserializeRecommendationDecision(
        ...
        try {
            ...
            throw new IllegalStateException(
                    "Gemini returned invalid recommendation reasoning JSON: "
                            + responseText,
                    exception
            );
        }
